...
for sig in Wh #WZ 
do
 python mergeTrees.py -v ${ntuple_path}/${tag}/*.merged --filters filters/filters_ht.json --weights weights_qqbb.json --did-to-group did_to_group.json --output-suffix _${tag}_${sig} --groups-bkg "" --groups-sig ${sig} --do-jms-fix --dids `cat processes/*.list  | tr '\n' ' '` > ${sig}.log 2>&1 &
done
