...
def do_summary(args):
  ...
  num_cores = min(multiprocessing.cpu_count(),args.num_cores)
  logger.log(25, "Using {0} cores".format(num_cores) )
  # results = Parallel(n_jobs=num_cores)(delayed(utils.get_summary)(filename, mass_windows, args.stop_masses, args.do_ewk) for filename in glob.glob(os.path.join(args.search_directory, "s*.b*.json")))
  results = (utils.get_summary(filename, mass_windows, args.stop_masses, args.do_ewk) for filename in glob.glob(os.path.join(args.search_directory, "s*.b*.json")))
...
def main():
  ...
  optimize_parser.add_argument('--maxSigStatUncertainty', type=float, required=False, dest='maxSigStatUncertainty', metavar='<sigma>', help='maximum signal statistical uncertainty for calculating significance', default=0.5)
  optimize_parser.add_argument('--insignificanceSignal', type=float, required=False, dest='insignificanceThresholdSignal', metavar='<min events>', help='minimum number of signal events for calculating significance', default=0.5)
  optimize_parser.add_argument('--insignificanceBkgd', type=float, required=False, dest='insignificanceThresholdBkgd', metavar='<min events>', help='minimum number of background events for calculating significance', default=0.5)
  ...
  summary_parser.add_argument('-o', '--output', required=False, type=str, dest='output', help='Output json to make', default='summary.json')
  summary_parser.add_argument('--stop-masses', required=False, type=int, nargs='+', help='Allowed stop masses', default=[5000])
  summary_parser.add_argument('--do-ewk', action='store_true', help='Do summary for electroweak SUSY searches')
