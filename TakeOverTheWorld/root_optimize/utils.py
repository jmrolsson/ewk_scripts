...
did_regex = re.compile('(?:00)?([0-9]\d{5})(?=\.[a-zA-Z_]+\.?)')
...
#@echo(write=logger.debug)
def get_significance(signal, bkgd, insignificanceThresholdSignal, insignificanceThresholdBkgd, bkgdUncertainty, bkgdStatUncertainty, rawBkgd):
  # if not enough events, return string of which one did not have enough
  if signal < insignificanceThresholdSignal:
    #sigDetails['insignificance'] = "signal"
    sig = -1
  elif bkgd < insignificanceThresholdBkgd:
...
#@echo(write=logger.debug)
def get_lumi(counts_type, lumi):
    if 'scaled' in counts_type:
        return lumi*1000.0
    else:
        return 1.0
...
def get_summary(filename, mass_windows, stop_masses=[], do_ewk=False):
