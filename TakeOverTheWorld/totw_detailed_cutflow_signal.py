...
def save_detailed_cutflow_signal(cuts_sig, cuts_bkg, tag='<cutflow>', outfile='detailed_cutflow.txt', do_latex=False):
  ...
  try:
    ...
    if do_latex:
      ...
      print(r"retain-explicit-plus=false, group-digits = true}", file=f)
    ...
    print(toprule, file=f)
    if do_latex:
      print("\multicolumn{{11}}{{c}}{{ {} }} \\\\[0.2cm]".format(tag), file=f)
    else:
      print(tag, file=f)
    print(toprule, file=f)
    if do_latex:
    ...
    for i, (cut, s) in enumerate(ordered_cuts):
      ...
      if do_latex:
          # Signal
          cutflow_str = "{} & \\num{{{:.1f}}} & \\num{{{:.1uS}}} & \\num{{{:g}}} & \\num{{{:g}}}".format(
          cut, ufloat(s.n, s.err), ufloat(s.wsn, s.wserr), seff, sreleff)
          # Background
          cutflow_str += " & \\num{{{:.1f}}} & \\num{{{:.1uS}}} & \\num{{{:g}}} & \\num{{{:g}}} & \\num{{{:g}}} & \\num{{{:g}}}\\\\".format(
    ...
    if do_latex:
      ...
      # caption_str += r"The raw number of events ($N_{S,B}^{\mathrm{raw}}$), the number of events scaled to cross section ($N_{S,B}$), "
      ...
      # caption_str += r"[cite] function $\Zn = {\mathrm{BinomalExpZ}}(N_S,N_B, 0.30)$, with an estimated flat uncertainty of $30\%$. "
  ...
  try:
    ...
    with tempfile.NamedTemporaryFile() as tmpFile:
      ...
      for path in cutflows_paths.keys():
        ...
        hbkg_scaled = reduce(lambda x,y: getattr(x, y, None), (item for item in path.split('/') if item != args.topLevel), bkgall)
        hsig_scaled = reduce(lambda x,y: getattr(x, y, None), (item for item in path.split('/') if item != args.topLevel), sigall)

        scale_hists(hbkg_scaled, weights, backgrounds)
        scale_hists(hsig_scaled, weights, signals)

        bkg_hists_scaled = map(lambda hgroup: hgroup.flatten, hbkg)
        sig_hists_scaled = map(lambda hgroup: hgroup.flatten, hsig)

        # background cutflows
        cut_bkg = Cut()
        for h, hs in zip(bkg_hists, bkg_hists_scaled):
          ...
          # create a cut
          cut_bkg.add(n, err, wn, werr, wsn, wserr)
        ...
        # signal cutflows
        for h, hs in zip(sig_hists, sig_hists_scaled):
          # raw number of events
          n = h.GetEntries()
          err = np.sqrt(n)
          # sum of event weights
          ...
          wserr = hs.GetBinError(1)
          # create a cut
          cut_sig = Cut(n, err, wn, werr, wsn, wserr)
          key = h.GetTitle()
          if key in cuts_sigs:
      ...
      if args.detailed_cutflow:
        for label, cut_sigs in cuts_sigs.iteritems():
          outfile = signals[label]['outfile']
          print("----> Saving detailed cutflow: ", outfile)
          print("tag", label)
          save_detailed_cutflow_signal(cut_sigs, cuts_bkg, label, outfile, args.do_latex)
