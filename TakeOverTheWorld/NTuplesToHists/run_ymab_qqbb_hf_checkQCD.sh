...
while read did; do
 ...
 python YourMacrosAreBadAndYouShouldFeelBad.py --config config/config_qqbb_hf_checkQCD${cuts_tag}.json --out_tdirectory hists --treename nominal --eventWeight ${event_weight} --filters filters/filters_ht.json --newOutputs --copyMBJCutflow -o ${ntuple_path}/${tag}-histograms/hf_checkQCD${cuts_tag}${filter}/ ${ntuple_path}/${tag}/*${did}*.${suffix} &
...
#   python YourMacrosAreBadAndYouShouldFeelBad.py --config config/config_qqbb_hf_checkQCD${cuts_tag}.json --out_tdirectory hists --treename nominal --newOutputs --copyMBJCutflow --doData -o ${ntuple_path}/${tag}-histograms/hf_checkQCD${cuts_tag}${filter}/ ${ntuple_path}/${tag}/*${period}*.merged &
# done;
